...
/**
 * Test script using native Deno KV API
...
async function testNativeDenoKV() {
  console.log("🔗 Testing native Deno KV...");
  console.log(`📍 Server: ${KV_URL}`);
  console.log(`🔑 Token: ${ACCESS_TOKEN.substring(0, 8)}...`);
  console.log("");

  try {
    // Connect to remote DenoKV using native API
    ...
    try {
      ...
      kv = await Deno.openKv(KV_URL);
      console.log("✅ Connected using direct URL");
    } catch (error) {
      console.log("❌ Direct URL connection failed:", error.message);
      
      try {
        // Method 2: With access token
        kv = await Deno.openKv(KV_URL, { accessToken: ACCESS_TOKEN });
        console.log("✅ Connected using URL + access token");
      } catch (error2) {
        console.log("❌ URL + token connection failed:", error2.message);
        
        try {
          // Method 3: Environment variable approach
          Deno.env.set("DENO_KV_ACCESS_TOKEN", ACCESS_TOKEN);
          kv = await Deno.openKv(KV_URL);
          console.log("✅ Connected using environment variable");
        } catch (error3) {
          console.log("❌ Environment variable connection failed:", error3.message);
    ...
    const testValue = "Hello from native Deno KV! 🚀";
    ...
    // Write 10 entries (reduced for faster testing)
    for (let i = 0; i < 10; i++) {
    ...
    console.log(`   - 20 operations (10 writes + 10 reads)`);
    ...
    console.log("");
    console.log("🎉 All native Deno KV tests completed successfully!");
    ...
    console.error("   - Try updating Deno to the latest version");
